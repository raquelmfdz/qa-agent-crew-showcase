...
def build_tasks(agents: dict[str, Agent]) -> list[Task]:
    ...
    reporting_task = Task(
        description=(
            ...
            "into a release-quality Markdown report. Include: an executive "
            "summary, a coverage summary (risks covered vs total), "
            "pass/fail totals, the top open risks (including any test "
            "failures found), and a section headed 'GO / NO-GO "
            "Recommendation' with your verdict **bolded** on its own (e.g. "
            "'**GO**' or '**NO-GO**') followed by your reasoning -- the "
            "bolded verdict is rendered as a highlighted badge, so it must "
            "be exactly GO or NO-GO with no other words inside the bold "
            "markers.\n"
            "Write Markdown only -- do not write HTML. Use the 'Write "
            "release report' tool with that Markdown content to save it; "
            "the tool automatically generates a matching styled HTML page "
            "for GitHub Pages. Do not just print the report."
        ),
        expected_output=(
